...
# funcao para enviar diretamente para o job-receiveitor-master na chococino
...
EOF
    #cat $TEMP >&2
    cat $TEMP | timeout 60 nc ${CHOCOCINO/:??*} ${CHOCOCINO/??*:/} | jq -r '.jobid' > $TEMP.a
    CODIGO=$(<$TEMP.a)
    echo "$CHOCOCINO-$CODIGO" |tr ':' ','
    echo "$CHOCOCINO-$CODIGO" >&2
    echo "=== $CODIGO" >&2
    rm $TEMP.a $TEMP
    ## Gambiarra horrível
    ...
    echo "${CHOCOCINO/:??*} ${CHOCOCINO/??*:/} $CODIGO" > $HOME/contests/$COMPETICAO/mojlog/$LOCALID
    MOJCONTESTSERVERS=""
    CONTEST_TYPE=""
...
function pega-resultado-cdmoj()
{
    ...
    local RESULT
    local SLEEPTIME
    echo "{ \"cmd\": \"getresult\", \"jobid\": \"$JOBID\" }" | timeout 30 nc ${PORT/:??*} ${PORT/??*:/} | jq -r '.status' > $TEMP
