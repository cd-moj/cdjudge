...
function cmd-listproblems()
{
  ...
  ls */*/tl| while read f; do
    grep -q '^PUBLIC=no' $PROBLEMDIR/${f/\/tl/}/conf && continue
    [[ -d "${f/\/tl/}/docs" ]] || continue
    [[ -d "${f/\/tl/}/tests" ]] || continue
    [[ -d "${f/\/tl/}/sols" ]] || continue
    local HORATL=$(stat -c %Y $f|| echo 0)
    local HORADOC=$(stat -c %Y ${f/\/tl/}/docs/enunciado.*|head -n1|| echo 0)
    local HORATAG=$(stat -c %Y ${f/\/tl/}/tags)
    local HORA=$HORATL
    (( HORADOC > HORA )) && HORA=$HORADOC
    (( HORATAG > HORA )) && HORA=$HORATAG
    (( first == 0 )) && echo -n ','
    (( first == 1 )) && first=0
    echo "[\"${f%%/tl}\",$HORA]"
  done
...
function cmd-getproblemhtml()
{
  cd $PROBLEMDIR
  f=$(echo $JSON|jshon -e param|tr -d '"'|tr -d '\\')
  if [[ ! -d "$PROBLEMDIR/$f" ]] || grep -q PUBLIC=no $PROBLEMDIR/$f/conf; then
  ...
  cd $(dirname $f)
  PANDOCEXTRAFLAG="-M document-css=false" make $(basename $f).html &>/dev/null
  echo "\"html\": \"$(base64 -w0 < $(basename $f).html)\"",
  echo "\"tags\": \"$(base64 -w0 < $(basename $f)/tags)\"",
  if [[ -e $PROBLEMDIR/$f/docs/enunciado.tex ]]; then
	  echo "\"title\": \"$(grep -E 'section{' $PROBLEMDIR/$f/docs/enunciado*|cut -d'{' -f2-|head -n1|cut -d'}' -f1|tr -d '\r')\""
  else
	  echo "\"title\": \"$(grep -E '^(%|.+[tT][iI][tT][lL][eE]:)' $PROBLEMDIR/$f/docs/enunciado*|cut -d' ' -f2-|head -n1|tr -d '\r')\""
  fi
  echo "}"
...
for f in run updateproblemset getresultfull getresult report reportmachine reporttls problemtl listproblems islocked getproblemhtml; do
