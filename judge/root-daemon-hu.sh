...
PROBLEMDIR=$(realpath /home/prof/judge/problems.hu/)
...
for LOG in $LOGDIR/*; do
  ...
  echo ":$CONT"
  ((CONT++))
  egrep -q '(On queue|Running)' $LOG/status && echo "Nil" > $LOG/status
done
fi
...
while true; do
  for ID in *; do
    ...
    flock $FREEMACHINE taskset -c 0-23 bash $HOME/mojtools/build-and-test.sh\
